...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc Displays a bust image instead of selected face image
 * 
 ...
 * the bust script will instead use /img/pictures/Actor1_2.png bust image.
 *
...
Galv.MB.bustPos = function(pos) {
	if (pos[0] === "TRUE") {
		return $gameMessage.bustDisable = false;
	} else if (pos[0] === "FALSE") {
		return $gameMessage.bustDisable = true;
	};
	
	$gameMessage.bustPos = 0
	if (pos[0] === "LEFT") {
		$gameMessage.bustPos = 0;
	} else if (pos[0] === "RIGHT") {
		$gameMessage.bustPos = 1;
	};
	if (pos[1] === "TRUE") {
		$gameMessage.bustMirror = true;
	} else if (pos[1] === "FALSE") {
		$gameMessage.bustMirror = false;
	};
};
...
var Galv_Game_Message_setFaceImage = Game_Message.prototype.setFaceImage;
Game_Message.prototype.setFaceImage = function(faceName, faceIndex) {
	var faceName = faceName.toLowerCase();
	switch (faceName) {
		case 'partyleader':
    ...
    Galv_Game_Message_setFaceImage.call(this,faceName,faceIndex);
};
...
Window_Message.prototype.startMessage = function() {
	...
	$gameMessage.bustPos = $gameMessage.bustPos || 0;
	$gameMessage.bustOffset = $gameMessage.bustOffset || Galv.MB.w;
	Galv_Window_Message_startMessage.call(this);
	$gameMessage.msgWindow.tempPosType = this._positionType;
};


var Galv_Window_Message_processEscapeCharacter = Window_Message.prototype.processEscapeCharacter;
Window_Message.prototype.processEscapeCharacter = function(code, textState) {
	...
	Galv_Window_Message_processEscapeCharacter.call(this, code, textState);
};
...
var Galv_Window_Message_drawMessageFace = Window_Message.prototype.drawMessageFace;
Window_Message.prototype.drawMessageFace = function() {
	if (!$gameMessage.bustDisable) return;
	Galv_Window_Message_drawMessageFace.call(this);
};
...
// UNDER MESSAGE
	...
	var Galv_Window_Message_newLineX = Window_Message.prototype.newLineX;
	Window_Message.prototype.newLineX = function() {
		if ($gameMessage.bustDisable) {
			return Galv_Window_Message_newLineX.call(this);
		} else {
			return 0;
		};
	};
...
// OVER MESSAGE
	...
	var Galv_Window_Message_createSubWindows = Window_Message.prototype.createSubWindows;
	Window_Message.prototype.createSubWindows = function() {
		Galv_Window_Message_createSubWindows.call(this);
		if (this._msgBustSprite) return;
		this._msgBustSprite = new Sprite_GalvBust();
		this.addChild(this._msgBustSprite);
		//this.newPage();
	...
	Window_Message.prototype.newLineX = function() {
		if ($gameMessage.bustDisable) {
			return Galv_Window_Message_newLineX.call(this);
		} else if ($gameMessage.faceName() && Galv.MB.prio == 1 && $gameMessage._positionType == 2 && $gameMessage.bustPos == 0) {
...
Sprite_GalvBust.prototype.update = function() {
    ...
    if ($gameMessage.msgWindow && !$gameMessage.bustDisable) this.controlBitmap();
};
...
Sprite_GalvBust.prototype.controlBitmap = function() {
	...
	if ($gameMessage.msgWindow.openness <= 0 || !this.hasBust) {
		this.opacity = 0;
		this.name = "";
		this.hasBust = false;
		return;
	...
	this.opacity = $gameMessage.faceName() ? $gameMessage.msgWindow._openness : this.opacity - 32;
	
	// Control image position
	switch ($gameMessage.msgWindow.tempPosType) {
	case 0:
	case 1:
	//top and middle
		this.y = this.baseY();
	...
	//bottom
		if (Galv.MB.prio == 1) {
			this.y = $gameMessage.msgWindow.height - this.bitmap.height;
		} else if (Galv.MB.pos === 1) {
			this.y = this.baseY();
		} else {
			this.y = this.baseY() - $gameMessage.msgWindow.height;
		};
		break;
	};
	...
	if ($gameMessage.bustPos == 1) {
		// if on the right
		this.x = Graphics.width - this.bitmap.width + offset;
	} else {
		// else on the left
		this.x = 0 + offset;
	};
